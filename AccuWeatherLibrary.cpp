...
static const std::map<const String, const AccuParserTokens> stringToTokenMap = {
  ...
  { "GmtOffset", ACCUPARSERGMTOffset},
  { "TimeZone", ACCUPARSERTimezone},
  { "IsDaylightSavings", ACCUPARSERIsDaylightSavings},
  { "Code", ACCUPARSERCode},
  { "Name", ACCUPARSERName},
  { "PrimaryPostalCode", ACCUPARSERPostalCode},
  { "GeoPosition", ACCUPARSERGeoPosition},
  { "Latitude", ACCUPARSERLatitude},
  { "Longitude", ACCUPARSERLongitude},
  { "LocalizedName", ACCUPARSERLocalizedName},
};
...
static const AccuParserTokens locationGmtOffset_suffix[] = {ACCUPARSERTimezone, ACCUPARSERObject, ACCUPARSERGMTOffset};
static const AccuParserTokens locationName_suffix[] = {ACCUPARSERLocalizedName};
static const AccuParserTokens locationPostalCode_suffix[] = {ACCUPARSERPostalCode};
static const AccuParserTokens locationTimeZoneCode_suffix[] = {ACCUPARSERTimezone, ACCUPARSERObject, ACCUPARSERCode};
static const AccuParserTokens locationTimeZoneName_suffix[] = {ACCUPARSERTimezone, ACCUPARSERObject, ACCUPARSERName};
static const AccuParserTokens locationTimeZoneIsDaylightSavings_suffix[] = {ACCUPARSERTimezone, ACCUPARSERObject, ACCUPARSERIsDaylightSavings};
static const AccuParserTokens locationLatitude_suffix[] = {ACCUPARSERGeoPosition, ACCUPARSERObject, ACCUPARSERLatitude};
static const AccuParserTokens locationLongitude_suffix[] = {ACCUPARSERGeoPosition, ACCUPARSERObject, ACCUPARSERLongitude};
...
AccuweatherLocationParser::AccuweatherLocationParser(AccuweatherLocationData * data_ptr_) : AccuweatherParser(1) {
  data_ptr = data_ptr_;
}

void AccuweatherLocationParser::value(String value) {
  if (baseListIdx >= maxListLength) {
    return;
  }

  if (STACK_HAS_SUFFIX(locationName_suffix)) {
    if (data_ptr->Name == NULL)
      data_ptr->Name = value;
  } else if (STACK_HAS_SUFFIX(locationGmtOffset_suffix)) {
    data_ptr->GmtOffset = value.toInt();
  } else if (STACK_HAS_SUFFIX(locationPostalCode_suffix)) {
    data_ptr->PostalCode = value;
  } else  if (STACK_HAS_SUFFIX(locationTimeZoneCode_suffix)) {
    data_ptr->TimeZoneCode = value;
  } else if (STACK_HAS_SUFFIX(locationTimeZoneName_suffix)) {
    data_ptr->TimeZoneName = value;
  } else if (STACK_HAS_SUFFIX(locationTimeZoneIsDaylightSavings_suffix)) {
    data_ptr->IsDaylightSavings = value;
  } else if (STACK_HAS_SUFFIX(locationLatitude_suffix)) {
    data_ptr->Latitude = value.toFloat();
  } else if (STACK_HAS_SUFFIX(locationLongitude_suffix)) {
    data_ptr->Longitude = value.toFloat();
  }
  popAllKeys();
}
...
static const char locationURLTemplate[] PROGMEM = "http://dataservice.accuweather.com/locations/v1/%d?apikey=%s&details=false&language=%s";
int Accuweather::getLocation(AccuweatherLocationData * data_ptr) {
  char url[256];
  snprintf_P(url, 256, locationURLTemplate, locationID, apiKey, languageID);
  http.begin(url);
  int httpCode = http.GET();
  if (httpCode > 0) {
    if (httpCode == HTTP_CODE_OK) {
      length = http.getSize();
      parser.reset();
      listener = new AccuweatherLocationParser(data_ptr);
      parser.setListener(listener);
      return 0;
    }
  }
